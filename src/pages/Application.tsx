...
const applicationSchema = z.object({
  ...
  notes: z.string().trim().max(100).optional()
}).refine((data) => {
  // Token required for all types except donation
  if (data.applicationType !== "donation" && !data.notes) {
    return false;
  }
  return true;
}, {
  message: "Access token is required for this application type",
  path: ["notes"]
});
...
const Application = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Skip token validation for donations
        ...
        duration: 0.5
          ...
          <div className="bg-white border-2 border-border rounded-2xl p-8 md:p-12 shadow-lg">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {formData.applicationType !== "donation" && (
                <div className="space-y-2">
